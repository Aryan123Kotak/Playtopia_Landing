...
export default function Hero() {
  ...
  return (
    <section id="top" ref={ref} onMouseMove={onMove} onMouseLeave={onLeave} className="relative min-h-[100svh] overflow-hidden pt-28 sm:pt-32 pb-20" data-testid="hero-section">
      {/* soft color glows (spotlight aurora) */}
      ...
      {/* floating parallax shapes (scroll + mouse) */}
      ...
      <motion.div style={{ opacity: heroOpacity }} className="relative z-10 mx-auto grid max-w-7xl items-center gap-10 px-5 sm:px-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          ...
          <div className="mt-16 max-w-xl text-2xl sm:text-4xl font-bold tracking-tight">
            <Line>Rediscover Play.</Line>
            <Line className="text-pink">Rediscover People.</Line>
          </div>
          ...
          <motion.div initial={false} className="mt-7 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm sm:text-base font-bold uppercase tracking-wide">
            ...
            <span className="inline-flex items-center gap-2"><MapPin className="h-5 w-5 text-coral" /> {EVENT.venue}</span>
          </motion.div>

          <motion.div initial={false} className="mt-8 flex flex-wrap items-center gap-4">
            <PlayButton href={TICKET_URL} color="pink" testid="hero-register-btn"><Ticket className="h-5 w-5" /> Register Here</PlayButton>
            <a href="#momentum" data-testid="hero-scroll-link" className="inline-flex items-center gap-2 text-sm font-extrabold uppercase tracking-wide">
        ...
        {/* hero image, clipped frame + parallax */}
